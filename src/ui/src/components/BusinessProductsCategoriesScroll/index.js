...
import { Tabs, Tab, AutoScroll, useWindowSize } from '~ui'

const BusinessProductsCategoriesScrollUI = (props) => {
  const {
    isSkeleton,
    categories,
    featured,
  ...
  const [selectedCategory, setSelectedCateogry] = useState({ id: selectedCategoryId })
  ...
  const handleScroll = () => {
    ...
    _categories?.length && _categories.some(category => {
      const topPos = category?.id
        ? document.getElementById(`category${category.id}`)?.offsetTop
    ...
    if (windowSize.width >= 993) {
      limit = window.scrollY >= wrapperCategories?.offsetTop && window.scrollY > 0
    } else {
      limit = window.scrollY >= (wrapperCategories?.offsetTop - 56 - 65) && window.scrollY > 0
    }
  ...
  useEffect(() => {
    ...
    style0 += `top: ${useKioskApp ? '0px' : '0px'} !important;`
  ...
  useEffect(() => {
    if (selectedCategoryId) {
      handleChangeCategory({ id: selectedCategoryId })
    }
  }, [selectedCategoryId])
...
export const BusinessProductsCategoriesScroll = (props) => {
  const businessProductsCategoriesProps = {
    ...props,
    UIComponent: BusinessProductsCategoriesScrollUI
  }
