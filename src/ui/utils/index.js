import { CODES, useLanguage } from '~components'
...
export const getTraduction = key => {
  ...
  return keyList[key] ? t(key, keyList[key]) : (/^[^\s]*_[^\s]*$/.test(key)) ? t(key, capitalize(key.replace(/_/g, ' ').toLowerCase())) : t(key.replace(/ /g, '_').toUpperCase(), key)
}
/**
